...
#define BUFSIZE 20
    ...
    static Logbuf iringbuf[BUFSIZE];
    static int logbuf_index = 0;
    Logbuf *head = &iringbuf[0];
    Logbuf *tail = &iringbuf[BUFSIZE - 1];

    if (logbuf_index < BUFSIZE)
    {
...
#if defined(CONFIG_ITRACE) || defined(CONFIG_IRINGTRACE_COND)
  char *p = s->logbuf;
