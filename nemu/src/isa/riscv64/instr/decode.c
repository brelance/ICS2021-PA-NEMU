...
{
  sword_t sval = 0;
  sval |= s->isa.instr.b.imm4_1 << 1;
  sval |= s->isa.instr.b.simm10_5 << 5;
...
def_THelper(riscv64_imm)
{
  def_INSTR_TAB("??????? ????? ????? 000 ????? ????? ??", addiw); // I type
  // TODO: without test
  def_INSTR_TAB("?0????? ????? ????? 001 ????? ????? ??", slliw);
...
def_THelper(main)
{
  ...
  def_INSTR_IDTAB("??????? ????? ????? ??? ????? 01110 11", R, riscv64_reg);
  def_INSTR_IDTAB("??????? ????? ????? ??? ????? 00110 11", I, riscv64_imm);
